...
export function TripCalculator({ totalCollected, memberCount, onClose }: TripCalculatorProps) {
  ...
  return (
    <div className="fixed inset-0 bg-[#070b14] text-white z-[100] flex flex-col overflow-hidden max-w-md mx-auto font-sans shadow-2xl touch-manipulation">

      {/* Top Navigation */}
      <div className="absolute top-4 left-4 z-[110]">
        <button
          onClick={() => onClose ? onClose() : window.location.href = '/?tab=home'}
          className="w-12 h-12 bg-black/20 backdrop-blur-md rounded-full flex items-center justify-center border border-white/10 active:scale-95 transition-transform touch-manipulation"
        >
          ...
          <div className="bg-slate-900 border border-white/10 w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95">
            <div className="p-6 border-b border-white/5 flex justify-between items-center">
              ...
              <button 
                ...
                className="text-slate-500 hover:text-white min-w-[44px] min-h-[44px] flex items-center justify-center active:scale-95 transition-all touch-manipulation"
              >
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="py-4 bg-slate-800 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-700 active:scale-95 transition-all flex items-center justify-center gap-2 touch-manipulation min-h-[52px]"
                >
                  <span>📋</span> Salin Teks
                </button>
                <button
                  onClick={confirmShare}
                  className="py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-500 active:scale-95 transition-all shadow-lg shadow-emerald-600/20 flex items-center justify-center gap-2 touch-manipulation min-h-[52px]"
                >
      ...
      <div className="flex-1 px-5 py-4 space-y-7 overflow-y-auto no-scrollbar pb-10">
        ...
        <div className="bg-indigo-900/10 border border-indigo-500/10 rounded-2xl p-4 flex justify-between items-center mt-2">
          ...
          <div className="bg-slate-900/60 border border-slate-800 px-2 rounded-xl flex items-center gap-2 py-1">
            <button 
              onClick={() => setManualMemberCount(Math.max(1, manualMemberCount - 1))} 
              className="text-slate-400 hover:text-white transition-colors text-xl font-light min-w-[44px] min-h-[44px] flex items-center justify-center active:scale-95 touch-manipulation"
            >
              −
            </button>
            <input
            ...
            />
            <button 
              onClick={() => setManualMemberCount(manualMemberCount + 1)} 
              className="text-slate-400 hover:text-white transition-colors text-xl font-light min-w-[44px] min-h-[44px] flex items-center justify-center active:scale-95 touch-manipulation"
            >
              +
            </button>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <div className="bg-slate-900/60 border border-slate-800 px-1 rounded-xl flex items-center gap-1">
              <button 
                onClick={() => setNights(Math.max(1, nights - 1))} 
                className="text-slate-500 text-lg hover:text-white transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center active:scale-95 touch-manipulation"
              >
                −
              </button>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center px-1">
            ...
            <div className="flex gap-1.5">
              <button 
                ...
                className="px-3 py-2.5 bg-indigo-600/10 text-indigo-300 border border-indigo-500/20 rounded-lg text-[9px] font-medium uppercase tracking-wider active:scale-95 transition-transform touch-manipulation min-h-[44px]"
              >
                + Mobil
              </button>
              <button 
                onClick={() => addVehicle('motorcycle')} 
                className="px-3 py-2.5 bg-emerald-600/10 text-emerald-300 border border-emerald-500/20 rounded-lg text-[9px] font-medium uppercase tracking-wider active:scale-95 transition-transform touch-manipulation min-h-[44px]"
              >
                + Motor
              </button>
            </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-0.5 no-scrollbar min-h-[38px]">
            {vehicles.map(v => (
              <div key={v.id} className="flex-shrink-0 bg-slate-900/40 border border-slate-800/60 rounded-lg px-2.5 py-2 flex items-center gap-2 group animate-in zoom-in-95">
                <span className="text-base">{v.type === 'car' ? '🚗' : '🏍️'}</span>
                <button 
                  onClick={() => setVehicles(prev => prev.filter(x => x.id !== v.id))} 
                  className="text-slate-600 hover:text-red-400 transition-colors min-w-[32px] min-h-[32px] flex items-center justify-center active:scale-95 touch-manipulation"
                >
        ...
        <div className="space-y-3 font-sans">
          <div className="flex justify-between items-center px-1">
            ...
            <button 
              ...
              className="px-3 py-2.5 bg-blue-600/10 text-blue-400 border border-blue-500/20 rounded-lg text-[9px] font-medium uppercase hover:bg-blue-600/20 transition-all active:scale-95 touch-manipulation min-h-[44px]"
            >
              + Baru
            </button>
          </div>
            ...
            {additionalCosts.map(c => (
              <div key={c.id} className="bg-slate-900/40 border border-slate-800/30 rounded-xl p-3 flex justify-between items-center animate-in slide-in-from-left-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <button 
                    onClick={() => setAdditionalCosts(prev => prev.filter(x => x.id !== c.id))} 
                    className="text-slate-600 ml-1 min-w-[32px] min-h-[32px] flex items-center justify-center active:scale-95 touch-manipulation hover:text-red-400 transition-colors"
                  >
                    ×
                  </button>
                </div>
              ...
              <div key={cat.id} className="space-y-3">
                <div className="flex justify-between items-center px-1">
                  ...
                  <button
                    ...
                    className={`text-${cat.color}-400 text-[9px] font-bold tracking-tight bg-${cat.color}-400/10 px-3 py-2.5 rounded-md border border-${cat.color}-400/20 active:scale-95 transition-all touch-manipulation min-h-[44px]`}
                  >
                    + {cat.title}
                  </button>
                </div>
                      ...
                      <tbody className="divide-y divide-white/5 italic">
                        {items.length === 0 ? (
                          ...
                          <tr key={item.id} className="hover:bg-white/[0.01]">
                            ...
                            <td className="py-1.5 px-1 text-center">
                              <button 
                                onClick={() => setConsumptionItems(prev => prev.filter(x => x.id !== item.id))} 
                                className="text-slate-700 hover:text-red-500 transition-colors text-base min-w-[32px] min-h-[32px] flex items-center justify-center active:scale-95 touch-manipulation"
                              >
                                ×
                              </button>
                            </td>
          ...
          <div className="pt-2 border-t border-white/10 space-y-3">
            <div className="flex justify-between items-center bg-emerald-500/5 p-4 rounded-2xl border border-emerald-500/10">
              ...
              <button
                ...
                className="bg-emerald-500 text-white px-4 py-3.5 rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95 transition-all flex items-center justify-center gap-2 touch-manipulation min-h-[48px]"
              >
      ...
      <div className="p-6 pb-8 shrink-0 bg-[#070b14]">
        <button
          ...
          className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold text-xs uppercase tracking-widest shadow-xl shadow-indigo-500/10 active:scale-95 transition-all text-center flex items-center justify-center gap-2 touch-manipulation min-h-[52px]"
        >
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl animate-in slide-in-from-bottom-10">
            <div className="p-6 pb-2 flex justify-between items-center">
              ...
              <button
                ...
                className="w-10 h-10 flex items-center justify-center bg-slate-800 rounded-full text-slate-400 active:scale-95 transition-transform touch-manipulation"
              >
                ×
              </button>
            </div>
            ...
            <div className="p-6 pt-2 space-y-3">
              <button
                ...
                className="w-full bg-emerald-500 text-white py-4 rounded-2xl font-bold text-xs uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95 transition-all flex items-center justify-center gap-3 touch-manipulation min-h-[52px]"
              >
              ...
              <button
                onClick={() => setShowSharePopup(false)}
                className="w-full bg-slate-800 text-slate-400 py-3.5 rounded-2xl font-bold text-[10px] uppercase tracking-widest active:scale-95 transition-all touch-manipulation min-h-[48px]"
              >
                Kembali Edit
              </button>
            </div>
