...
export function TripCalculator({ totalCollected, memberCount }: TripCalculatorProps) {
  ...
  const [additionalCosts, setAdditionalCosts] = useState<AdditionalCost[]>([
    { id: 'm', name: 'Total Makan', amount: 2000000 },
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      const data = { selectedVillaId, manualVillaPrice, manualMemberCount, vehicles, carFuel, motorFuel, tollCost, nights, additionalCosts };
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
    }, 500);
    return () => clearTimeout(timeoutId);
  }, [selectedVillaId, manualVillaPrice, manualMemberCount, vehicles, carFuel, motorFuel, tollCost, nights, additionalCosts]);
  ...
  const grandTotal = villaTotal + totalFuelCost + totalTollCost + totalAdditionalCosts;
  ...
  const generateWSMessage = () => {
    ...
    return `*RINCIAN ESTIMASI VILLA TRIP Family Gathering Maganghub* 😹
...
🚗 *Transportasi:*
- Armada: ${carCount} Mobil, ${motorCount} Motor
...
_Dikirim via Villa Trip Manager_`.trim();
  };

  const handleWhatsAppShare = () => {
    const encodedMessage = encodeURIComponent(generateWSMessage());
